...
const MaskedHeading = ({
  ...
  const place = useCallback(() => {
    ...
    const s = settingsRef.current as { fillScale: number; parallax: number; drift: number; brightness: number; saturation: number; grayscale: boolean; textScale: number };
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      // Pause while the heading is off-screen; the observer restarts us.
      if (!visible) {
        raf = 0;
        return;
      }
          ...
          <span ref={mediaRef} className="masked-heading__media">
            {mediaType === "video" ? (
              <video
                ...
                preload="auto"
                onError={(e) => {
                  // Retry transient media load failures (e.g. concurrent mounts of the same src)
